 ...
 */
package com.heracles.framework.filter;
 ...
 * 
 * 在web.xml中配置的参数包括：
 * 1.loginFailureUrl -- 身份验证失败后跳转的URL, 需与SpringSecurity中的配置保持一致, 无默认值必须配置.
 ...
 * 6.registerFailureUrl --注册验证失败后跳转的URL
 * 
 * 
...
public class JCaptchaFilter implements Filter {
	...
	public static final String PARAM_LOGIN_FAILURE_URL = "loginFailureUrl";
	public static final String PARAM_REGISTER_FAILURE_URL = "registerFailureUrl";
	...
	private String registerFailureUrl;
	...
	protected void initParameters(final FilterConfig fConfig) {
		if (StringUtils.isBlank(fConfig.getInitParameter(PARAM_LOGIN_FAILURE_URL)) && StringUtils.isBlank(fConfig.getInitParameter(PARAM_REGISTER_FAILURE_URL))) {
			throw new IllegalArgumentException("CaptchaFilter缺少failureUrl参数");
		}

		loginFailureUrl = fConfig.getInitParameter(PARAM_LOGIN_FAILURE_URL);
		registerFailureUrl = fConfig.getInitParameter(PARAM_REGISTER_FAILURE_URL);
	...
	public void doFilter(final ServletRequest theRequest, final ServletResponse theResponse, final FilterChain chain)
		...
		//加入全局的DispatchServices
		if (DispatchServices.getRequest() == null){
			DispatchServices.setRequest(request);
		}
	}
	...
	protected void redirectFailureUrl(final HttpServletRequest request, final HttpServletResponse response)
		...
		if (StringUtils.startsWith(servletPath, filterProcessesUrl)) {
			...
			response.sendRedirect(request.getContextPath() + registerFailureUrl);
		}
	}
